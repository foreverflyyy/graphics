...
void CAboutDlg::OnMouseMove(UINT nFlags, CPoint point)
{
	...
	HICON hIcon;	
	CRect IconRect;
	// Координаты иконки в единицах диалогового окна
	IconRect.left=11; IconRect.top=17;
	IconRect.right=IconRect.left+20; IconRect.bottom=IconRect.top+20;
	// Преобразовать координаты иконки в экранные координаты
	MapDialogRect(&IconRect);
	// Если попали в иконку
	if(IconRect.PtInRect(point))
		hIcon=AfxGetApp()->LoadIcon(IDR_MAINFRAME1); // загрузить измененную копию
	else hIcon=AfxGetApp()->LoadIcon(IDR_MAINFRAME); // загрузить оригинал
	// Получим указатель элемент интерфейса, показывающий иконку
